...
import net.dxs.videoplayer.widget.CustomVideoView;
...
public class VideoPlayerActivity2 extends Activity implements
	...
	public void onConfigurationChanged(Configuration newConfig) {
		...
		if (newConfig.orientation == Configuration.ORIENTATION_LANDSCAPE) {// 横屏
			...
			setVideoViewScale(ViewGroup.LayoutParams.MATCH_PARENT,
					ViewGroup.LayoutParams.MATCH_PARENT);// 设置为全屏
	...
	private void updateTimeFormat(TextView tv, int millisecond) {
		...
		int second = s % 60;
		tv.setText(String.format(Locale.CHINA, "%02d:%02d:%02d", hour, minute,
				second));
	}
	...
	private void changeBrightness(float offset) {
		...
		brightness = Math.max(brightness + index,
				WindowManager.LayoutParams.BRIGHTNESS_OVERRIDE_OFF);
		brightness = Math
				.min(WindowManager.LayoutParams.BRIGHTNESS_OVERRIDE_FULL,
						brightness);
